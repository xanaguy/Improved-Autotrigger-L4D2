...
public Action:CheckAroundTriggers(Handle:timer)
{
	...
	if (StrContains(mapname, "c7m1_docks", false) != -1)
	{
		...
		if (CheckforBots(posx, 200.0) && !WarpTrigger)
		{
			...
			new Handle:posdata = CreateDataPack();
			WritePackFloat(posdata, 4825.021484);
			WritePackFloat(posdata, 818.668396);
			WritePackFloat(posdata, 176.031250);
			CreateTimer(4.0, WarpAllBots, posdata);
			WarpTrigger = true;
		}
	}
...
public Action:RunBusStationEvent2(Handle:Timer)
{
	...
	//PrintToChatAll("\x04[AutoTrigger] \x01They run for the tower...");
}
/*
